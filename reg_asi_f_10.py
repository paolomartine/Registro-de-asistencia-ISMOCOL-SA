...
def buscar():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT nombre, cargo FROM trabajadores WHERE cedula=?", (cedula,))
...
<canvas id="canvas" style="border:1px solid black; width:350px; height:200px;"></canvas>
<br><br>
...
function guardar(){
    ...
    }).then(()=>alert("Firma guardada correctamente, cero papel"));
}

function verPDF(){
    window.open("/ver_pdf/{{cedula}}", "_blank");
}
</script>
<br><br>
...
def guardar():
    ...
    pdf_path = f"pdfs/{cedula}.pdf"

    buffer = BytesIO()
    c = pdfcanvas.Canvas(buffer, pagesize=A4)
    width, height = A4
    c.drawString(50, height - 50, f"Cédula: {cedula}")
    c.drawString(50, height - 70, "Documento firmado:")
    firma_img = BytesIO(firma_bytes)
    c.drawImage(firma_img, 50, height - 300, width=200, height=100, mask='auto')
    c.showPage()
    c.save()
...
@app.route("/ver_pdf/<cedula>")
def ver_pdf(cedula):
    return send_file(f"pdfs/{cedula}.pdf", mimetype="application/pdf", as_attachment=False)
...
def reporte_final():
    ...
    # -------- ENCABEZADO CON LOGO --------
    logo_path = "/content/logoismocol.png"  # Asegúrate de subir el logo con este nombre

    if os.path.exists(logo_path):
        c.drawImage(logo_path, 40, height - 90, width=120, height=50, mask='auto')
    ...
    c.drawString(180, height - 78, f"Fecha de generación: {fecha}")

    y = height - 110

    # -------- ENCABEZADO TABLA --------
    c.setFont("Helvetica-Bold", 9)
    c.drawString(40, y, "Cédula")
    c.drawString(120, y, "Nombre")
    c.drawString(260, y, "Cargo")
    c.drawString(400, y, "Firma")
    y -= 15
    ...
    for cedula, nombre, cargo, firma in rows:
        ...
        firma_clean = firma.split(",")[1] if "," in firma else firma
...
<body>
  ...
  <p>Tu firma fue guardada correctamente.</p>
