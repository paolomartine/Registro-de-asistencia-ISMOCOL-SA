...
# --- MIGRACION: agregar columna fecha_firma si no existe ---
conn = sqlite3.connect(DB_PATH)
cur = conn.cursor()
try:
    cur.execute("ALTER TABLE trabajadores ADD COLUMN fecha_firma TEXT")
    conn.commit()
    print("✅ Columna fecha_firma creada")
except sqlite3.OperationalError:
    print("ℹ️ La columna fecha_firma ya existe")
conn.close()
...
def guardar_firma_asistente():
    ...
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("UPDATE trabajadores SET firma=? WHERE cedula=?", (firma_base64, cedula))
    conn.commit()
    conn.close()
...
def reporte_final():
    ...
    # Asistentes en orden real de firma
    cur.execute("""
